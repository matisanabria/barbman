...
public class AdvanceRepositoryImpl implements AdvanceRepository {
    ...
    public void save(Advance advance) {
        ...
        String sql = """
            ...
            (user_id, amount, date, payment_method_id, expense_id, description)
            VALUES (?, ?, ?, ?, ?, ?)
        """;
        ...
        try (Connection db = DbBootstrap.connect();
            ...
            ps.setString(6, advance.getDescription());
    ...
    public void update(Advance advance) {
        ...
        String sql = """
            UPDATE advances
            SET user_id = ?, amount = ?, date = ?, payment_method_id = ?, expense_id = ?, description = ?
            WHERE id = ?
        """;
    ...
    public List<Advance> findByUserAndDateRange(int userId, LocalDate from, LocalDate to) {
        ...
        try (Connection db = DbBootstrap.connect();
            ...
            logger.debug("{} Found {} advances for user {} between {} and {}",
                    PREFIX, list.size(), userId, from, to);
    ...
    public double getTotalByUserAndDateRange(int userId, LocalDate from, LocalDate to) {
        ...
        try (Connection db = DbBootstrap.connect();
            ...
            logger.info("{} Total advances for user {} between {} and {} = {}",
                    PREFIX, userId, from, to, total);
